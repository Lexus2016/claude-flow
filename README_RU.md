...
**Используйте любой LLM с Claude Code** — OpenRouter, DeepSeek, OpenAI, Gemini или пользовательские endpoint.
...
Доступно на: **[English](README.md)** | [Українська](README_UA.md) | **[Русский](README_RU.md)**

---

## Проблема

Claude Code CLI **по умолчанию работает только с API Anthropic**. Если вы хотите использовать OpenRouter, DeepSeek, OpenAI или другого провайдера, нужно правильно установить переменные окружения.

Есть **критическая ловушка, которая ломает большинство конфигураций**: `ANTHROPIC_API_KEY` должна быть установлена на пустую строку (`""`), а не отсутствовать. Если она отсутствует — Claude Code выдает ошибку. Если она непустая — он игнорирует ваш токен аутентификации провайдера.

**claude-flow решает это правильно** — и обрабатывает все остальные переменные окружения. Установите один раз и забудьте.
...
| **DeepSeek** | V3, R1 (рассуждение) | Самый дешевый | Легко | Экономное рассуждение |
| **OpenAI** | GPT-4.1, o3 | Премиум | Легко | Передовые модели |
| **Gemini** | Gemini 2.5 Pro/Flash | Конкурентный | Легко | Быстрый вывод |
...
DeepSeek V3 (быстрый) и R1 (рассуждение) через их родной API. Стоимость в несколько центов.
...
```bash
claude-flow setup deepseek
# При появлении приглашения для Sonnet и Opus используйте: deepseek-reasoner
```
...
GPT-4.1 и o3 (рассуждение) через официальный API OpenAI.
...
Gemini 2.5 Pro и Flash через API Google.
...
| `ANTHROPIC_API_KEY` | API ключ (прямые провайдеры) | `sk-...` |
| `ANTHROPIC_DEFAULT_HAIKU_MODEL` | Модель быстрого уровня | `anthropic/claude-haiku-4` |
| `ANTHROPIC_DEFAULT_SONNET_MODEL` | Сбалансированная модель | `anthropic/claude-sonnet-4` |
| `ANTHROPIC_DEFAULT_OPUS_MODEL` | Мощная модель | `anthropic/claude-opus-4` |

### Критическая ловушка

**`ANTHROPIC_API_KEY` должна быть установлена на пустую строку (`""`) для провайдеров-прокси, а не отсутствовать или не быть установленной.**

Это вся причина существования claude-flow.
...
#### Пример: DeepSeek (использует api_key)
...
1. **Определяет тип провайдера** (прокси или прямой)
2. **Для провайдеров-прокси** (OpenRouter): Устанавливает `ANTHROPIC_API_KEY = ""` и `ANTHROPIC_AUTH_TOKEN = apiKey`
3. **Для прямых провайдеров** (DeepSeek, OpenAI): Устанавливает `ANTHROPIC_API_KEY = apiKey`
4. **Всегда устанавливает** `ANTHROPIC_BASE_URL` и три переменные `ANTHROPIC_DEFAULT_*_MODEL`
5. **Возвращает полный объект env** готовый передать Claude Code
...
const env = buildEnv('openrouter', {
  apiKey: 'sk-or-v1-...',
  model: 'deepseek/deepseek-chat'  // Все уровни используют это
});
```
