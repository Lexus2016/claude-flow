...
**Користуйтеся будь-яким LLM з Claude Code** — OpenRouter, DeepSeek, OpenAI, Gemini або власні endpoint'и.
...
Claude Code CLI **за замовчуванням працює тільки з API Anthropic**. Якщо ви хочете користуватися OpenRouter, DeepSeek, OpenAI або іншим провайдером, потрібно встановити змінні середовища правильно.

Є **критичний "подвох", який ламає більшість настроєнь**: `ANTHROPIC_API_KEY` повинна бути встановлена на порожний рядок (`""`), а не відсутня. Якщо вона відсутня — Claude Code видає помилку. Якщо вона непорожна — він ігнорує ваш токен автентифікації провайдера.

**claude-flow вирішує це правильно** — і обробляє всі інші змінні середовища. Встановіть один раз і забудьте.
...
| **DeepSeek** | V3, R1 (reasoning) | Найдешевше | Легко | Дешеві міркування |
| **OpenAI** | GPT-4.1, o3 | Преміум | Легко | Найновіші моделі |
| **Gemini** | Gemini 2.5 Pro/Flash | Конкурентна | Легко | Швидке виконання |
...
DeepSeek V3 (швидкий) та R1 (міркування) через їх власний API. Односимвольні вартості.
...
```bash
claude-flow setup deepseek
# При запиті для Sonnet та Opus використовуйте: deepseek-reasoner
```
...
GPT-4.1 та o3 (міркування) через офіційний API OpenAI.
...
Gemini 2.5 Pro та Flash через API Google.
...
| `ANTHROPIC_API_KEY` | API ключ (прямі провайдери) | `sk-...` |
| `ANTHROPIC_DEFAULT_HAIKU_MODEL` | Модель швидкого рівня | `anthropic/claude-haiku-4` |
| `ANTHROPIC_DEFAULT_SONNET_MODEL` | Модель збалансованого рівня | `anthropic/claude-sonnet-4` |
| `ANTHROPIC_DEFAULT_OPUS_MODEL` | Модель потужного рівня | `anthropic/claude-opus-4` |

### Критичний подвох

**`ANTHROPIC_API_KEY` повинна бути встановлена на порожний рядок (`""`) для прокси-провайдерів, а не відсутна.**

Це вся причина існування claude-flow.
...
export ANTHROPIC_BASE_URL='https://openrouter.ai/api'
export ANTHROPIC_AUTH_TOKEN='sk-or-v1-...'
export ANTHROPIC_API_KEY=''  # Порожний рядок, не відсутня
export ANTHROPIC_DEFAULT_SONNET_MODEL='anthropic/claude-sonnet-4'
...
#### Приклад: DeepSeek (використовує api_key)
...
### Як claude-flow це робить правильно

Функція `buildEnv()` в `lib/env.js`:

1. **Визначає тип провайдера** (прокси vs. прямий)
2. **Для прокси-провайдерів** (OpenRouter): встановлює `ANTHROPIC_API_KEY = ""` та `ANTHROPIC_AUTH_TOKEN = apiKey`
3. **Для прямих провайдерів** (DeepSeek, OpenAI): встановлює `ANTHROPIC_API_KEY = apiKey`
4. **Завжди встановлює** `ANTHROPIC_BASE_URL` та три `ANTHROPIC_DEFAULT_*_MODEL` змінні
5. **Повертає повний об'єкт env** готовий для передачі до Claude Code
...
const env = buildEnv('openrouter', {
  apiKey: 'sk-or-v1-...',
  model: 'deepseek/deepseek-chat'  // Всі рівні використовують це
});
```
