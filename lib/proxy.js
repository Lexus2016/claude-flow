...
class SanitizingProxy {
   ...
   */
  constructor({ targetUrl, authHeader = "Authorization", authValue = "", verbose = false, maxOutputTokens = 0 }) {
  ...
  async _handleMessages(req, res) {
    ...
    const sanitizeOpts = this._maxOutputTokens ? { maxOutputTokens: this._maxOutputTokens } : undefined;
    const sanitized = sanitizeRequestBody(body, sanitizeOpts);
